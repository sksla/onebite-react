...
- 특정 목적을 갖는 프로그램의 단위
- 예 : 온라인 쇼핑몰을 만든다 -> 쇼핑몰 프로젝트를 만든다, 포털 사이트 만든다 -> 웹 포털 프로젝트를 만든다
- Node.js에서 프로젝트를 부르는 다른 말 ==> **패키지(Package)**

### - 패키지(Package)

- Node.js에서 사용하는 프로그램의 단위
- Node.js 환경에서는 여러 개의 JavaScript 파일로 어떠한 목적을 갖는 프로그램을 만들 때 모두 패키지라는 단위로 프로그램을 만듦
- 우리가 만들 모든 JavaScript 앱, 모든 React 앱들도 이런 패키지 단위로 만들어지게 됨
- 우리가 앞으로 사용할 모든 라이브러리들도 패키지라는 단위로 만들어져 있음

#### - 따라서 우리가 앞으로 Node.js를 이용해서 JavaScript로 무언가를 만들기 위해서는 가장 먼저 패키지라는 것을 이해하고 사용할 줄 알아야 한다!

## 실습 : 간단한 Node.js 패키지 만들어 보기

### 1. 패키지의 루트 폴터 생성

- Node 패키지를 생성하려면 패키지의 뿌리가 될 폴더 즉 루트 폴더를 하나 만들어줘야 됨
- 이번에 사용할 패키지를 위한 새로운 폴더 만들어 줌 ex) `ONEBITE-REACT/section03`

### 2. VSCode에서 생성한 루트 폴더를 열어줌

- 이 상태에서 루트 폴더를 뿌리로 하는 새로운 Node.js 패키지를 생성해주면 됨

### 3. 패키지 생성

#### - 터미널을 이용해 패키지 생성

- 별도의 터미널을 열어줄 필요 없이 VSCode 내에서 자체적으로 제공되는 터미널을 이용하면 훨씬 더 편리하다
- VSCode 터미널 여는 단축키 :
  - Windows: Ctrl + J
  - macOS: Command + J
- VSCode 내부에서 터미널을 열면 자동으로 이 터미널의 작업 경로가 루트 폴더 내부로 설정됨

- 터미널에 새로운 패키지를 초기화 또는 생성해달라는 명령어 입력 :

  ```bash
  npm init
  ```

- 새로운 패키지 생성을 위한 몇가지 정보를 물어봄 (엔터로 넘어가기)

  - `package name: (기본본값)` : 생성하려는 패키지 이름
    - 기본값은 루트 폴더명
    - 예 :
      ```bash
  ...
  - `descrption: ` : 패키지의 간단한 설명 입력
    - 없으면 생략 가능
  - `entry point: (index.js)` : 메인으로 실행될 JS 파일
    - 실습 중엔 그냥 넘어감
  - `test command: ` : test command가 무엇인지
    - 테스트 배운 적 없으므로 엔터로 넘어감
  - `git repository: ` : git repository를 물어봄
    - git이 없으니 엔터로 넘어감
  - 이후 `keyword: `, `license` 등 다 엔터로 넘어감
  - `Is this OK? (yes)`: 마지막에 설정한 새로운 패키지의 정보가 이게 맞냐라고 물어봄
    - 여기서도 엔터를 입력해서 패키지 생성 완료함

- 패키지 생성이 완료되면 루트 폴더(`/section03`) 내에 `package.json`이라는 새로운 설정 파일이 자동으로 생성됨
  - 이 파일엔 패키지를 생성할 때 설정한 기본적인 정보들이 잘 저장되어 있음

#### - `section03` 이라는 패키지 안에 새로운 JavaScript 파일 생성 및 Node.js로 실행

- 우클릭 + 새 파일로 새로운 `index.js` 라는 JavaScript 파일 생성 후 간단한 자바스크립트 코드 작성

  ```javascript
  console.log("안녕 Node.js");
  ```

- 자바스크립트 코드를 브라우저가 아닌 Node.js를 이용해서 실행시키기

  - 터미널에 `node` + 실행시키고 싶은 파일의 경로와 이름 입력 :
  ...
  - 패키지 안에 계속해서 코드를 작성하다 보면 새로운 파일, 폴더들이 계속 생성된다 그러다보면 Node로 실행하고자하는 파일의 경로가 계속 복잡해질 수 있음
  - 이럴 때 패키지 스크립트를 이용하면 좋다

#### - 패키지 스크립트

- `package.json` 파일에서 `scripts`라는 항목 안에 들어있는 일종의 매크로들
- `"test"`라는 기존에 설정된 스크립트 아래 `start`라는 새로운 스크립트를 만들고 콜론(:), 큰따옴표(")로 이 스크립트의 값을 `node src/index.js`라고 설정해준다
  ```
  "scripts": {
...
- 스크립트 실행 방법
  ...
  - 터미널에 `npm run`이라는 명령어 뒤에 `start` 입력 :
    ...
    npm run start
    ```
  - 우리가 설정한 `node src/index.js`가 실행됨

- 패키지 내부의 파일과 경로가 복잡해지게 될 경우에는 이런식으로 `scripts`라는 패키지의 기능을 이용해서 복잡한 경로에 있는 파일도 Node를 통해서 실행하도록 명령할 수 있다!
